...
04. `{}`  Code block
05. `[]`  Sequence and map, generic types
06. `|`   Union data type 
07. `->`  Function declaration
08. `//`  Nothing-check operator
09. `:`   Type declaration (binding, struct field and function inputs), type alias
...
**Data types**: `int`, `float`, `char`, `byte`, `bool`, `string`, `nothing`, `type`, `struct`, `enum`

**Reserved identifiers**: `true`, `false`, `fn`, `import`, `and`, `or`, `not`

## Coding style

1. 4 spaces indentation.
2. You must put each statement on a separate line. 
3. Naming: `SomeDataType`, `someLambdaBinding`, `someFunction`, `any_data_binding`, `my_module`.
...
Bindings that start with udnerscore are designated as private (Although they are accessible form outside).
...
Bindings of a union type, have ability to hold multiple different types and are shown as `T1|T2|T3|...`. 

You can use core function `hasType` to check if a union has a specific type in it (Example 3). Also there is `check` function which can be used to implement type matching for unions (Example 4). You can also use a map to run a code based on type inside a union binding (Example 5). In this case, compiler will make sure that your map has covered all possible types.

**Examples**

1. `int_or_float: int|float = 11`
2. `has_int = hasType(int, int_or_float)`, `int_or_nothing_value = tryCast(int|float, int, int_or_float)`
3. 
```
...
```
4. `int_or_nothing = tryCast(int_or_float, int)`
5. `x = [int: fn(x:int|float->string)..., float: fn(x:int|float->string)...][type(int_or_float)](int_or_float)`
...
You can use built-in function `cast` and `tryCast` to cast primitives, union types and named types.

**Examples**

1. `MyInt = int`, `int_val = cast(my_int_val, int)`
2. `int_val = cast(age_float, int)`


## Generics

Generic types are defined using functions that return a `type` (a type argument) and use `[]` instead of `()` . These functions must be compile time calculatable (because anything related to `type` must be) (Example 1). 
...
The result of importing a module is a module definition which if named, should be named like a binding and used with `::` notation to access definitons inside module. You can also ignore output of an import to have its definitions inside current namespace.

Bindings defined at module level must have compile time calculatable values.

Absolute paths that start with http or https will be downloaded from the net if not available locally.

You can also import multiple modules at the same time using sequence notation after module path (Example 8). 

You can import at module level or inside a function. The output of import will be valid only inside its enclosing context.

You can use `@` notation to indicate required tag or branch name. This part allows using `+` and `*` to indicate versions equal or higher to x or any version are acceptable (Example 9).
