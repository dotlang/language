...
You can treat type name as a function to cast a union to one of its component types (which will return that type or nothing), or a named type to its component or to cast primitives.

**Examples**

1. `MyInt = int`, `int_val = MyInt(my_int_val)`
2. `int_val = int(age_float)`
