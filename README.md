...
11. `:`   Type declaration (binding, struct field and function inputs) and type alias
12. `=`   Binding declaration
13. `_`   Place-holder (lambda creator and assignment)
14. `@`   Import
15. `::`  Return
...
We have `:=` for parallel execution of an expression. This will give you a task identifier which is a unique identifier for the parallel task. It also contains the result of the expression.
