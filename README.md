...
a = 10
a = 1.1
x:float = a #without type specifier, compiler does not know which "a" is intended.
```
8.
```
process = (x:int)->x+1
...
Bindings defined at module level must be compile time calculatable. You can also change or add definitions when importing a module (Syntax 2, Example 8).
...
- **Version 1.00**: ??? ??, ???? - Use `=` for type alias and `:=` for lazy (parallel) calculation and named type, More clarification about binding type inference, explain name resolution mechanism for types and bindings and function call
